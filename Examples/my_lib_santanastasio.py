...
def lin_fit(x, y, sd_y, xlabel="x [ux]", ylabel="y [uy]", xm=0., xM=1., ym=0., yM=1., 
    ...
    if (plot):
        ...
        plt.errorbar(x, y, yerr=sd_y, xerr=0, ls='', marker='o', 
                     color="b", label='punti sperimentali')
        ...
        y1_plus_1sigma = y1+2*y_estrapolato(x1, m, c, np.sqrt(var_m), np.sqrt(var_c), cov_mc)[1]
        y1_minus_1sigma = y1-2*y_estrapolato(x1, m, c, np.sqrt(var_m), np.sqrt(var_c), cov_mc)[1]         
        plt.plot(x1,y1_plus_1sigma, linestyle='-', color="r", label=r'fit $\pm 2\sigma$')
        plt.plot(x1,y1_minus_1sigma, linestyle='-', color="r")
